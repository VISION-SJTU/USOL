...
def evaluate(model, writer, index, dataset_val, opt):
        ...
        os.makedirs(os.path.join('.', 'vis_folder', opt.name,'epoch_'+str(index)))
        os.makedirs(os.path.join('.', 'raw_folder', opt.name,'epoch_'+str(index)))
        raw_folder = os.path.join('.', 'raw_folder', opt.name, 'epoch_'+str(index))
        vis_folder = os.path.join('.', 'vis_folder', opt.name, 'epoch_'+str(index))
        val_losses = []
        val_unsup_losses = []
        val_bot_losses = []
        with torch.no_grad():
                for i, (frame_t_val, pos_audio_val, neg_audio_val, bottom_up_map_val, worker_gt_val, weights_t_val, datum_val) in enumerate(dataset_val):
                    #print('Eval step:',i)
                    bottom_up_map_val = bottom_up_map_val.to(opt.device)
                    ...
                    val_bot_loss = bottomup_loss_criteria(att_map_val, bottom_up_map_val)
                    val_total_loss = val_unsup_loss + val_bot_loss

                    val_losses.append(val_total_loss.item())
                    val_unsup_losses.append(val_unsup_loss.item())
                    val_bot_losses.append(val_bot_loss.item())
        ...
        avg_bot_loss = sum(val_bot_losses)/len(val_bot_losses)
        writer.add_scalar('data/val_bot_loss', avg_bot_loss, index)
        print('val bottom-up loss: %.7f' % avg_bot_loss)
        return avg_val_loss
...
dataset_train = Sound_Localization_Dataset(opt.dataset_file, 'train', '')
...
bottomup_loss_criteria = BottomupLoss()
best_err = float("inf")
...
for epoch in range(1 + opt.epoch_count, opt.niter+1):
        ...
        bottomup_loss = AverageMeter()
        unsupervised_loss = AverageMeter()
        batch_loss = AverageMeter()
        end_time = time.time()

        for i, (frame_t, pos_audio, neg_audio, bottom_up_map, worker_gt, weights_t, datum) in enumerate(dataloader_train):
                data_time.update(time.time() - end_time)
                bottom_up_map = bottom_up_map.to(opt.device)
                ...
                bot_loss = bottomup_loss_criteria(att_map, bottom_up_map)
                total_loss = unsup_loss + bot_loss # + sup_loss
                batch_loss.update(total_loss, frame_t.size(0))
                bottomup_loss.update(bot_loss, frame_t.size(0))
                ...
                print('Epoch: [{0}][{1}/{2}]\t'
                      ...
                      'Bot. Loss {bot_loss.val:.4f} ({bot_loss.avg:.4f})\t'
                      'Loss {total_err.val:.7f} ({total_err.avg:.7f})\t'.format(epoch, i + 1, len(dataloader_train), batch_time=batch_time, data_time=data_time, unsup_err=unsupervised_loss, bot_loss=bottomup_loss ,total_err=batch_loss))
        ...
        if(epoch % opt.display_freq == 0):
                ...
                avg_bottomup_loss = bottomup_loss.avg
                # print('supervised loss: %.7f' % avg_supervised_loss)
                print('bottom-up loss: %.7f' % avg_bottomup_loss)
                # writer.add_scalar('data/supervised_loss', avg_supervised_loss, epoch)
                writer.add_scalar('data/bottomup_loss', avg_bottomup_loss, epoch)
